...
static __strong NSDictionary        *dicColors;
...
- (UIColor *)colorFromName
{
    ...
    color = [colorsCache objectForKey:[self lowercaseString]];
    if (color)
        return color;

    NSString *key = self;
    NSString *colorCode = [dicColors objectForKey:[self lowercaseString]];
    if (colorCode)
    {
